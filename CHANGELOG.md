...
- Configurable authentication for a self-hosted sync server behind a reverse proxy (#183): set `PERGAMON_SYNC_BASIC_USER` and `PERGAMON_SYNC_BASIC_PASSWORD` for HTTP Basic auth, or `PERGAMON_SYNC_BEARER_TOKEN` for a bearer token, and the client sends the credential as an `Authorization` header on every sync request — you no longer have to embed it in the server URL. The same variables configure the web server's background sync worker when it syncs through a proxy. Credentials are read only from the environment, are marked sensitive so they are never logged, and are never written to your config file or local database
- Experimental multi-tenant server authentication for hosted sync using the OPAQUE aPAKE (#189): a self-hosted sync server started with `PERGAMON_SYNC_MODE=multitenant` now exposes account registration and login endpoints that authenticate a client without the server ever learning the password — the server stores only an OPAQUE verifier, never a password or password-equivalent, and the auth plane is cryptographically independent of your content keys (logging in proves identity but never yields the key that decrypts your library). There is deliberately no separate "does this account exist" endpoint: the account handle is returned only after a successful login, so an unauthenticated caller cannot tell a registered identity from an unknown one, and repeated failed logins for a single identity are slowed with exponential backoff. The default `blind` mode is unchanged and mounts none of these routes, so existing self-host deployments behave exactly as before. **This auth plane is a first implementation that has not yet been externally security-reviewed and must not be deployed to production until that review is complete;** a successful login does not yet mint a session token
- Per-device session tokens for the experimental multi-tenant sync server (#192): after a successful OPAQUE login, a server started with `PERGAMON_SYNC_MODE=multitenant` now mints a short-lived per-device access token (1 hour) and a longer-lived refresh token (30 days), each scoped to exactly one account and cryptographically bound to that device's key. Issuing a token requires the device to prove possession of its Ed25519 device key with a domain-separated, single-use signature tied to that specific login, so an unauthenticated caller can never obtain a token and a proof captured from one login cannot be replayed into another. A token minted for one account can never be presented as another account's. Tokens can be refreshed for a new access token — the refresh token itself is rotated on every use, so a captured refresh secret is good for only a single exchange — and a device can be revoked, after which all of its tokens are rejected on both validation and refresh. Only opaque token hashes are stored server-side, never the token secrets, so a database leak yields no usable tokens. The default `blind` mode is unchanged and exposes none of these endpoints. **Like the underlying auth plane, this is a first implementation that has not yet been externally security-reviewed and must not be deployed to production until that review is complete;** the tokens are not yet enforced on the content-sync routes

### Changed

- Self-hosted sync servers now handle concurrent devices and tenants without queueing them behind a single database connection (#201). The server stores data in SQLite WAL mode with one writer and a pool of reader connections, so reads run in parallel with each other and with writes, instead of every request — read or write, from any account — waiting on one process-wide lock. On a 14-core host, concurrent read throughput measured roughly **3.4× higher at the storage layer and 1.9× higher end to end**. To be explicit about the limit: SQLite permits exactly one writer at a time even in WAL mode, so this is a read-path improvement and write throughput is unchanged by design. New tuning knobs, all with defaults that keep a single-account self-host behaving as before: `PERGAMON_READ_POOL_SIZE` (default 8), `PERGAMON_STORE_CHECKOUT_TIMEOUT_MS` (default 5000), and a per-account concurrency cap (`PERGAMON_MAX_TENANT_CONCURRENCY`, derived from the pool by default) so one heavy account cannot monopolize every connection and stall the others. When the server is momentarily at capacity it now replies with a retryable `503` instead of a generic error
- **Operators: back up the whole sync-server data directory, not just `pergamon-sync.db`.** Because the database now runs in WAL mode, recently committed data can live in the `pergamon-sync.db-wal` sidecar file. The documented backup procedure (stop the container, archive all of `/data`, start it again) is still correct and needs no change, but a hand-rolled script that copies `pergamon-sync.db` on its own *while the server is running* now produces an incomplete backup (#201)
- Account creation is now an explicit, un-collapsible choice, and surfaces a recovery code by default (#188). `pergamon sync-device bootstrap` — the "create a new account" flow — now refuses to run on a device that already belongs to an account (pointing you to `sync-device enroll` or `sync-device recover` to *join* it instead), and, if the device already has a local library but no account yet, requires `--create-new-account` to confirm you really want a brand-new account rather than joining one you already have on another device. On success it now prints a high-entropy recovery code by default with a blunt warning that it is the only way back into the account if you lose every device and that the server cannot recover it for you; pass `--no-recovery-code` to skip it (recovery stays off until you run `sync-device recovery-enable`), or set `PERGAMON_RECOVERY_PASSPHRASE` to wrap recovery under your own passphrase instead. `pergamon device-key init` now notes that it created keys without a recovery code and points to `sync-device bootstrap`/`recovery-enable`. A fresh, local-only install still needs no account and no key ceremony — keys are only ever created by the `sync-device`/`device-key` commands
...
- Pre-auth abuse controls for the self-hosted sync server (#195): the server now defends its availability against an unauthenticated or newly-registered caller trying to exhaust it, with per-IP rate limiting (a generous default tier plus a stricter tier on the sensitive upload, event-push, and — in multi-tenant mode — registration/login routes), request and upload body-size caps, and a global concurrency limit that sheds excess load with `503` instead of letting one flooder starve everyone or run the host out of memory. All of it is configurable and off nothing by default: `PERGAMON_RATE_LIMIT_RPS`/`PERGAMON_RATE_LIMIT_BURST`, `PERGAMON_STRICT_RATE_LIMIT_RPS`/`PERGAMON_STRICT_RATE_LIMIT_BURST`, `PERGAMON_MAX_BODY_BYTES`, `PERGAMON_UPLOAD_MAX_BYTES`, and `PERGAMON_MAX_CONCURRENCY` (with matching `--flags`), and the defaults are deliberately generous so a normal self-host — including a first sync that uploads a whole library — is unaffected and the `/health` probe is never throttled. Rate limiting keys on the socket peer IP by default (so a client cannot spoof its address); set `PERGAMON_TRUST_PROXY_HEADERS`/`--trust-proxy-headers` to key on the real client IP from `X-Forwarded-For`/`X-Real-Ip`/`Forwarded` when running behind a trusted reverse proxy. These are aggregate and per-IP bounds, not per-tenant fairness quotas
- Enforced per-account authorization and tenant isolation on the experimental multi-tenant sync server (#197): in `PERGAMON_SYNC_MODE=multitenant` every account-scoped content and onboarding-relay route now requires a valid per-device bearer token (from #192) whose account matches the account the request targets, so the per-device tokens are finally enforced end-to-end. A missing, malformed, unknown, expired, or revoked token is rejected uniformly with `401` (the server never reveals which), and a valid token used against a *different* account — including the dedup existence probe, which would otherwise be a cross-tenant oracle — is denied with `403` and an audit log entry. The account identity routes are gated whether the account is named in the path, the request body, or the query string. The auth-establishing routes (register/login/token) and the `/health` probe stay open, and the default `blind` single-tenant mode is byte-for-byte unchanged — it mounts none of this and still requires no token. Enrolling a brand-new device is deliberately still allowed to read the roster and its wrapped keys so it can complete onboarding; per-device roster-membership enforcement and per-tenant fairness quotas remain future work. **Like the rest of the multi-tenant auth plane, this has not yet been externally security-reviewed and must not be deployed to production until that review is complete**
- Per-tenant storage accounting and enforceable quotas for the experimental multi-tenant sync server (#198): the server now meters each account's stored ciphertext — blob bytes/count and event bytes/count — and can cap it. Set `PERGAMON_MAX_ACCOUNT_BYTES`/`--max-account-bytes` and/or `PERGAMON_MAX_ACCOUNT_OBJECTS`/`--max-account-objects` (both default to `0`, meaning unlimited) and a write that would push an account past its cap is refused with `507 Insufficient Storage` and a `QUOTA_EXCEEDED` error naming which limit was hit; the batch is rolled back so nothing partial is stored. Reads (download, pull, dedup probe, usage) stay available while over quota so a tenant can export or delete to get back under, and re-uploading a blob the account already holds still succeeds because it adds nothing. Accounting is measured on ciphertext size and object counts only — never any decoded content — so it preserves the blind-relay model, and a new tenant-isolated `GET /v1/usage/{account}` endpoint reports an account's usage and quota status for billing/metrics (in multi-tenant mode it requires the account's own token; another account's token gets `403`). With no quota configured the default is unlimited, so blind self-hosts and existing multi-tenant deployments are byte-for-byte unchanged. **Like the rest of the multi-tenant control plane, this has not yet been externally security-reviewed and must not be deployed to production until that review is complete**
...
- Multi-device onboarding, account bootstrap, and key management for end-to-end-encrypted sync (#128): a new device can now join an existing account and restore the full library and review state without ever exposing the account key to the server. `pergamon sync-device` drives the ADR-024 flows — `bootstrap` founds a new account on the first device, `invite`/`enroll` publish a joining device's record, `approve`/`accept` complete an out-of-band-verified handoff (compare the shown Short Authentication String, then the approver seals the Account Root Key to the new device), `devices` lists the account roster, `revoke` rotates the key epoch and re-wraps to the remaining devices, and `recovery-enable`/`recover` provide an optional passphrase- or recovery-code-based restore for a device with no trusted peer. Accepting or recovering binds the device's sync identity and pulls the library automatically. The server only ever relays opaque, sealed artifacts
- `pergamon-sync` relay layer and onboarding orchestration (#128): a dependency-light `RelayTransport` trait (with an in-memory double for tests and a `reqwest`-backed `HttpRelay` behind the `http` feature) speaking the server's onboarding-relay endpoints, plus an `onboarding` module wiring the `pergamon-crypto` primitives into the bootstrap/enroll/approve/accept/revoke/recover flows. Canonical wire encodings (`to_bytes`/`from_bytes`) for signed device records and attestations let the full signed artifacts round-trip through the blind relay
- Background sync for the self-hosted web server (#129): the server can now run end-to-end-encrypted remote sync automatically in the background on a configurable interval, pushing changes made in the web UI and pulling remote ones, with exponential backoff and offline tolerance so a disconnected server keeps retrying rather than erroring out. It is opt-in — enable it with an encrypted key file and passphrase (`--sync-key-file`/`PERGAMON_SYNC_KEY_FILE`, `PERGAMON_SYNC_KEY_PASSPHRASE`, `--sync-account`, `--sync-interval`). Edits made in the browser (save, status triage, tag changes, bulk actions, and deletes) are now tracked so they are included in the next sync, and an admin endpoint (`POST /admin/sync-remote/trigger`) forces an immediate round
- Background refresh sync on iOS (#129): the app can now perform a sync from an iOS background-refresh task. After a one-time `configure_sync` with the account key (kept in the iOS keychain), each background wake runs a single push-and-pull round and returns how many changes moved plus a suggested delay before the next refresh — backing off automatically when the device is offline — so an item saved or triaged on one device shows up on another without manual steps
- `pergamon sync-remote daemon` CLI command (#129): runs continuous background sync on an interval with exponential backoff, retry, and offline tolerance (and an immediate first round on start), instead of having to invoke `push`/`pull`/`sync` by hand
- Docker image and Compose file for self-hosting the sync server (#130): a multi-stage `Dockerfile.sync-server` (minimal `debian:bookworm-slim` runtime, non-root user, statically linked SQLite) and `docker-compose.sync-server.yml` stand up `pergamon-sync-server` on port 8787 with a persistent `/data` volume and a built-in `health-check` probe. The AGPL-3.0 image is labelled accordingly and ships its license. A self-hosting guide (`docs/sync-server.md`) and [ADR-026](docs/adr/026-sync-server-deployment.md) cover quick start, configuration, reverse-proxy (Caddy/nginx) setup with TLS and authentication, connecting clients, backups, and the blind-relay security model; the Docker CI workflow now builds and publishes both the web and sync-server images
